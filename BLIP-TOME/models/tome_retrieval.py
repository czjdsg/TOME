...
class ToMe_Retrieval(nn.Module):
    def __init__(self,
        ...
        self.num_train_frames = model_cfg["num_frm_train"]

            
        self.enable_frame_pos = model_cfg['enable_frame_pos_embed']
        ...
        if model_cfg["token_merging"]:
            ...
            tome.patch.frame_mask_clip(self.visual_encoder,
            trace_source=model_cfg["trace_source"],
            r=model_cfg["tome_r"], 
            prop_attn=model_cfg["tome_prop_attn"], 
            type=model_cfg["tome_type"],
            min_token=model_cfg["tome_min_token"],
            hierarchy=model_cfg["tome_hierarchy"],
            token_importance=model_cfg["tome_token_importance"],
            importance_alpha=model_cfg["tome_importance_alpha"],
            frame_flatten_type=model_cfg["frame_flatten_type"],
            average_type=model_cfg["tome_average_type"],
            frame_flatten_layer=model_cfg["frame_flatten_layer"],
            enable_frame_mask=model_cfg["frame_mask"],
            attn_frame_mask_layer=model_cfg["attn_frame_mask_layer"],
            merge_frame_mask_layer=model_cfg["merge_frame_mask_layer"],
            binary_frame_mask=model_cfg["binary_frame_mask"],
            flatten_layer_merge_type=model_cfg["flatten_layer_merge_type"])
            
            tome.patch.frame_mask_clip(self.visual_encoder_m,
            trace_source=model_cfg["trace_source"],
            r=model_cfg["tome_r"], 
            prop_attn=model_cfg["tome_prop_attn"], 
            type=model_cfg["tome_type"],
    ...
    def forward(self, video, caption, alpha, idx, bsz=0):
        ...
        video_embeds, hidden_states = self.visual_encoder(video, self.num_train_frames, self.enable_frame_pos)
        video_embeds = rearrange(video_embeds, '(b n) l d -> b n l d', b=B)   
        if hasattr(self.visual_encoder, "_tome_info") and self.visual_encoder._tome_info["num_cls"] > 1:     
            video_feat = self.vision_proj(torch.mean(video_embeds[:, :, 0:self.visual_encoder._tome_info["num_cls"], :].flatten(1,2), dim=1))
        else:
            # print(video_embeds.shape)
            video_feat = self.vision_proj(torch.mean(video_embeds[:, :, 0, :], dim=1))  # mean pooling --> (batch_size, hidden_dim)
        video_feat = F.normalize(video_feat, dim=-1)
        video_embeds = rearrange(video_embeds, 'b n l d -> b (n l) d')
        ...
        with torch.no_grad():
            ...
            video_embeds_m, hidden_states_m = self.visual_encoder_m(video, self.num_train_frames)
            video_embeds_m = rearrange(video_embeds_m, '(b n) l d -> b n l d', b=B)
            if hasattr(self.visual_encoder_m, "_tome_info") and self.visual_encoder_m._tome_info["num_cls"] > 1:     
                video_feat_m = self.vision_proj_m(torch.mean(video_embeds_m[:, :, 0:self.visual_encoder_m._tome_info["num_cls"], :].flatten(1,2), dim=1))
            else:
                video_feat_m = self.vision_proj(torch.mean(video_embeds_m[:, :, 0, :], dim=1))  # mean pooling --> (batch_size, hidden_dim)
            video_feat_m = F.normalize(video_feat_m, dim=-1)
            video_embeds_m = rearrange(video_embeds_m, 'b n l d -> b (n l) d')
    ...
    def _momentum_update(self):
        for model_pair in self.model_pairs:
            # for name, param in model_pair[0].named_parameters():
...
def tome_retrieval(pretrained='', **kwargs):
    model = ToMe_Retrieval(**kwargs)
    if pretrained:
