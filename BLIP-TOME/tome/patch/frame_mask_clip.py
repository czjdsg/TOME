...
class ToMeResidualAttentionBlock(Block):
    def attention(self, x: torch.Tensor, size: torch.Tensor, frame_idx: torch.Tensor, binary_mask=False):
        ...
        if size is not None:
            size = size.log().unsqueeze(1).unsqueeze(1)[...,0] # N,1,1,L
            size = size.repeat(1, self.attn.num_heads, x.size(1), 1) # N*num_heads, L, L
        ...
        if "token_importance" in self._tome_info.keys():
            ...
            need_weights = False
        # print(self.attn.forward)
        attn_output, metric, attn_weights = self.attn(x, size, need_weights)
    ...
    def frame_flatten_plus(self, x, metric, frame_idx, attn_weights, video_frame):
        ...
        if merge_frame:
            ...
            x = self.frame_flatten_operation(x, video_frame, num_cls=num_cls, type=type, odd=video_frame%2==1, permute=False)
            ...
            if self._tome_info["size"] is not None:
                self._tome_info["size"] = self.frame_flatten_operation(self._tome_info["size"], video_frame, num_cls=num_cls, type=type, odd=video_frame%2==1, permute=False)
    ...
    def forward(self, x_dict:dict):
        ...
        x_attn, metric, attn_weights = self.attention(self.norm1(x), attn_size if self._tome_info["layer_idx"] > 0 else None, frame_idx if enable_attn_frame_mask else None, binary_mask=self._tome_info["binary_frame_mask"])
        ...
        if self._tome_info["layer_idx"] == 0:
            ...
            self._tome_info["tensor_shape"].append([video_frame, x.shape[1]])
...
class ToMeAttention(Attention):
    ...
    def forward(
        self, x: torch.Tensor, size: torch.Tensor = None, need_weights=False
