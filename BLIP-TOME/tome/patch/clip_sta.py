...
class ToMeResidualAttentionBlock(ResidualAttentionBlock):
    ...
    def sta_prune(self, x, metric, video_frame, depth=12):
        # x: L,BT,C
        # metric: BT,L,C
        r = self._tome_info["r"].pop(0)
        if r <= 0:
            return x
        x = x.transpose(0, 1).contiguous() # BT,L,C
        BT, L, _ = x.shape
        x = x.view(BT//video_frame, video_frame, L, -1)
        metric = metric.view(BT//video_frame, video_frame, L, -1)
        B = x.shape[0]  # bsz,TL,C
        index = self._tome_info["layer_idx"]
        
        if index == 0:
            perm = torch.arange(video_frame)
        elif index == depth // 3:
            perm = torch.arange(video_frame-1, -1, -1) 
        elif index == 2 * depth // 3:
            perm = torch.arange(video_frame)
        else:
            return x
        ...
        for t in range(1, video_frame):
            ...
            if index == depth // 3:
            	x_final = torch.cat([x_t[:,None], x_final], dim=1)             
            else:
            	x_final = torch.cat([x_final, x_t[:,None]], dim=1)   
        return x_final.view(BT, -1, D_x).transpose(0, 1).contiguous()
    ...
    def forward(self, x_dict:dict):
        ...
        if self._tome_info["layer_idx"] in [0, 4, 8]:
            x_cls, x_patch = x[:1], x[1:].contiguous()
            x_patch = self.sta_prune(x_patch, metric[:, 1:].contiguous(), video_frame)
