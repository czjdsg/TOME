...
from data.video_dataset import VideoDataset, caption_video, caption_video_eval, CoinDataset, QuerYD_train, \
    ...
    DiDeMo_retrieval_eval, MSVD_retrieval_train, MSVD_retrieval_eval, ActivityNet_qa, ActivityNet_retrieval_train, ActivityNet_retrieval_eval, \
    CondensedMovies_retrieval_train, CondensedMovies_retrieval_eval, ivqa, nextqa, flickr30k_train, flickr30k_retrieval_eval
...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset == 'pretrain':
        ...
        train_dataset = MSRVTT_retrieval_train(config['video_root'], config['data_root'], config['train_split_root'], num_frm=config['num_frm_train'],
                                               max_img_size=config['image_size'], frm_sampling_strategy='uniform',
                                               max_words=config['max_words'], split='train')
        val_dataset = MSRVTT_retrieval_eval(config['video_root'], config['test_split_root'], num_frm=config['num_frm_test'],
                                            max_img_size=config['image_size'], frm_sampling_strategy='uniform',
                                            max_words=config['max_words'], split='val')
        test_dataset = MSRVTT_retrieval_eval(config['video_root'], config['test_split_root'], num_frm=config['num_frm_test'],
                                             max_img_size=config['image_size'], frm_sampling_strategy='uniform',
                                             max_words=config['max_words'], split='test')
        return train_dataset, val_dataset, test_dataset
    
    elif dataset == 'retrieval_msvd':
        train_dataset = MSVD_retrieval_train(config['video_root'], config['ann_root'], num_frm=config['num_frm_train'],
                                               max_img_size=config['image_size'], frm_sampling_strategy='uniform',
                                               max_words=config['max_words'], split='train')
        val_dataset = MSVD_retrieval_eval(config['video_root'], config['ann_root'], num_frm=config['num_frm_test'],
                                            max_img_size=config['image_size'], frm_sampling_strategy='uniform',
                                            max_words=config['max_words'], split='val')
        test_dataset = MSVD_retrieval_eval(config['video_root'], config['ann_root'], num_frm=config['num_frm_test'],
                                             max_img_size=config['image_size'], frm_sampling_strategy='uniform',
                                             max_words=config['max_words'], split='test')
        return train_dataset, val_dataset, test_dataset
    ...
    elif dataset =='retrieval_flicker':
        train_dataset = flickr30k_train(config['video_root'], config['ann_root'], max_words=config['max_words'], image_size=config['image_size'])
        val_dataset = flickr30k_retrieval_eval(config['video_root'], config['ann_root'], split='val', max_words=config['max_words'], image_size=config['image_size'])
        test_dataset = flickr30k_retrieval_eval(config['video_root'], config['ann_root'], split='test', max_words=config['max_words'], image_size=config['image_size'])
        return train_dataset, val_dataset, test_dataset
