...
class MSRVTT_retrieval_train(Dataset):
    def __init__(self, video_root, data_root, split_root, num_frm=4, frm_sampling_strategy="rand", max_img_size=384,
                 video_fmt='.mp4', max_words=30, split='train'):
        '''
        image_root (string): Root directory of video
        ann_root (string): directory to store the annotation file
        '''
        self.train_df = pd.read_csv(split_root)
        self.db = load_json(data_root)
        self.compute_vid2caption()
        self.construct_all_train_pairs()
        ...
        # self.video2txt = {}
        # txt_id, video_id = 0, -1
        # for video_id in self.train_df['video_id']:
        #     video_path = os.path.join(self.video_root, video_id + self.video_fmt)
        #     if not os.path.exists(video_path):
        #         remove_cnt += 1
        #         continue
        #     if video_id not in video_ids:  # new video
        #         video_ids.add(video_id)
        #         video_id += 1
        #         self.video.append(video_id)
        #         self.video2txt[video_id] = []
        #     valid_annotation.append(video_id)
    ...
    def __getitem__(self, index):
        video_id, caption = self.all_train_pairs[index]
        video_path = os.path.join(self.video_root, video_id + self.video_fmt)
        vid_frm_array = load_video_from_path_decord(video_path, self.frm_sampling_strategy, self.num_frm, height=self.max_img_size, width=self.max_img_size)
        video = self.img_norm(vid_frm_array.float())
        id = int(video_id.strip('video'))

        return video, caption, id

    def construct_all_train_pairs(self):
        self.all_train_pairs = []
        for vid in self.train_df['video_id']:
            for caption in self.vid2caption[vid]:
                self.all_train_pairs.append([vid, caption])

    def compute_vid2caption(self):
...
class MSRVTT_retrieval_eval(Dataset):
    def __init__(self, video_root, split_root, num_frm=4, frm_sampling_strategy="rand", max_img_size=384,
                 video_fmt='.mp4', max_words=30, split='test'):
        
        self.test_df = pd.read_csv(split_root)
        self.text = []
        self.video = []
        self.txt2video = {}
        self.video2txt = {}
        ...
        for idx, data in self.test_df.iterrows():
            self.video.append(data['video_id'])
            self.text.append(data['sentence'])
            self.video2txt[idx] = [idx]
            self.txt2video[idx] = idx
    ...
    def __getitem__(self, index):
        video_path = os.path.join(self.video_root, self.test_df.iloc[index]['video_id'] + self.video_fmt)
        vid_frm_array = load_video_from_path_decord(video_path, self.frm_sampling_strategy, self.num_frm, height=self.max_img_size, width=self.max_img_size)
        video = self.img_norm(vid_frm_array.float())
        video_id = int(self.test_df.iloc[index]['video_id'].strip('video'))

        return video, video_id

class MSVD_retrieval_train(Dataset):
    def __init__(self, video_root, ann_root, num_frm=4, frm_sampling_strategy="rand", max_img_size=384,
                 video_fmt='.avi', max_words=30, split='train'):
        '''
        image_root (string): Root directory of video
        ann_root (string): directory to store the annotation file
        '''
        self.train_ids = load_txt(os.path.join(ann_root, f'{split}_list.txt'))
        self.ann = load_json(os.path.join(ann_root, 'captions_msvd.json'))
        self.video_id_to_idx = load_json(os.path.join(ann_root, 'video_id_to_idx.json'))
        self.construct_all_train_pairs()
    ...
    def __getitem__(self, index):
        ...
        vid_frm_array = load_video_from_path_decord(video_path, self.frm_sampling_strategy, self.num_frm, height=self.max_img_size, width=self.max_img_size)
        video = self.img_norm(vid_frm_array.float())
        id = self.video_id_to_idx[video_id]
        return video, caption, id

    def construct_all_train_pairs(self):
        self.all_train_pairs = []
        for vid in self.train_ids:
            if vid in self.video_id_to_idx.keys():
                for caption in self.ann[vid]:
                    self.all_train_pairs.append([vid, caption])

class MSVD_retrieval_eval(Dataset):
    def __init__(self, video_root, ann_root, num_frm=4, frm_sampling_strategy="rand", max_img_size=384,
                 video_fmt='.avi', max_words=30, split='test'):
        
        self.eval_ids = load_txt(os.path.join(ann_root, f'{split}_list.txt'))
        self.ann = load_json(os.path.join(ann_root, 'captions_msvd.json'))
        self.video_id_to_idx = load_json(os.path.join(ann_root, 'video_id_to_idx.json'))
        self.text = []
        self.video = []
        ...
        self.valid_eval_ids = []
        txt_id, video_id = 0, -1
        for vid in self.eval_ids:
            if vid in self.video_id_to_idx.keys():
                self.valid_eval_ids.append(vid)
    ...
    def __len__(self):
        return len(self.valid_eval_ids)
    
    def __getitem__(self, index):
        video_path = os.path.join(self.video_root, self.valid_eval_ids[index] + self.video_fmt)
        vid_frm_array = load_video_from_path_decord(video_path, self.frm_sampling_strategy, self.num_frm, height=self.max_img_size, width=self.max_img_size)
        video = self.img_norm(vid_frm_array.float())
        video_id = self.video_id_to_idx[self.valid_eval_ids[index]]
...
class DiDeMo_retrieval_train(Dataset):
    def __init__(self, video_root, ann_root, num_frm=4, frm_sampling_strategy="rand", max_img_size=384,
                 video_fmt='.mp4', max_words=30, split='train'):
        '''
        image_root (string): Root directory of video
        ...
        self.annotation = load_json(os.path.join(ann_root, filename))
        ...
        txt_id, video_id = 0, -1
        for ann in self.annotation:
            video_path = os.path.join(self.video_root, ann['video'] + self.video_fmt)
            if not os.path.exists(video_path):
                remove_cnt += 1
                continue
            if ann['video'] not in video_ids:  # new video
                video_ids.add(ann['video'])
                video_id += 1
                self.video.append(ann['video'])
                self.video2txt[video_id] = []
            valid_annotation.append(ann)
            self.text.append(ann['description'])
            self.video2txt[video_id].append(txt_id)
            self.txt2video[txt_id] = video_id
            txt_id += 1
        self.annotation = valid_annotation
    ...
    def __getitem__(self, index):
        ann = self.annotation[index]
        video_path = os.path.join(self.video_root, ann['video'] + self.video_fmt)
        vid_frm_array = load_video_from_path_decord(video_path, self.frm_sampling_strategy, self.num_frm, height=self.max_img_size, width=self.max_img_size, start_time=0, end_time=30)
        video = self.img_norm(vid_frm_array.float())
        video_id = range(len(self.annotation))[index]

        return video, ann['description'], video_id


        
class DiDeMo_retrieval_eval(DiDeMo_retrieval_train):
    def __init__(self, video_root, ann_root, num_frm=4, frm_sampling_strategy="rand", max_img_size=384,
                 video_fmt='.mp4', max_words=30, split='test'):
...
class flickr30k_train(Dataset):
    def __init__(self, image_root, ann_root, max_words=30, image_size=384):        
        '''
        image_root (string): Root directory of images (e.g. flickr30k/)
        ann_root (string): directory to store the annotation file
        '''        
        self.annotation = load_json(ann_root+'/flickr30k_train.json')
...
class flickr30k_retrieval_eval(Dataset):
    def __init__(self, image_root, ann_root, split, max_words=30, image_size=384):  
        '''
        image_root (string): Root directory of images (e.g. flickr30k/)
        ann_root (string): directory to store the annotation file
        split (string): val or test
        ...
        self.annotation = load_json(ann_root+f'/flickr30k_{split}.json')
        self.transform = transforms.Compose([
        ...
        for img_id, ann in enumerate(self.annotation):
            self.video.append(ann['image'])
            self.video2txt[img_id] = []
            for i, caption in enumerate(ann['caption']):
                self.text.append(pre_caption(caption,max_words))
                self.video2txt[img_id].append(txt_id)
                self.txt2video[txt_id] = img_id
                txt_id += 1
