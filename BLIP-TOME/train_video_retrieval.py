...
from models.tome_retrieval import tome_retrieval
import utils
...
import h5py
from weights_merge.merge import neuron_merge
...
def evaluation(model, data_loader, device, config, args):
    # test
    model.eval()
    # if config['weights_merging']:
    #     model = neuron_merge(model, config['weime_r'], config['weime_layers'])
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Evaluation:'
    ...
    mmap_file = None
    index = 0
    for video, vid_id in tqdm(data_loader):
        if video.dim() == 4:
            video = video.unsqueeze(1)
        video = video.permute(0, 2, 1, 3, 4)
        B, C, N, H, W = video.size()
        video = rearrange(video, 'b c n h w -> (b n) c h w')
        video = video.to(device, non_blocking=True)
        video_feat, _ = model.visual_encoder(video, config['num_frm_test'])  # [bsz*N, (image_size/patch_size)^2+1, 768]
        video_feat = rearrange(video_feat, '(b n) l d -> b n l d', b=B)
        if hasattr(model.visual_encoder, "_tome_info") and model.visual_encoder._tome_info["num_cls"] > 1:     
            video_embed = model.vision_proj(torch.mean(video_feat[:, :, 0:model.visual_encoder._tome_info["num_cls"], :].flatten(1,2), dim=1))
        else:
            # print(video_embeds.shape)
            video_embed = model.vision_proj(torch.mean(video_feat[:, :, 0, :], dim=1))  # mean pooling --> (batch_size, hidden_dim) 
        video_embed = F.normalize(video_embed, dim=-1)
        video_feat = rearrange(video_feat, 'b n l d -> b (n l) d')
        if args.low_resource_eval:
            if mmap_file is None:
                mmap_file = np.memmap('memmap_file.dat', dtype=np.float32, mode='w+', shape=(len(data_loader.dataset.video), video_feat.shape[1], video_feat.shape[2]))

            mmap_file[index:index+B, :, :] = video_feat.cpu().numpy()
            index += B
            print(index)
        else:
    ...
    if not args.low_resource_eval:
        video_feats = torch.cat(video_feats, dim=0)
    video_embeds = torch.cat(video_embeds, dim=0)

    sims_matrix_v2t = video_embeds @ text_embeds.t()
    print(sims_matrix_v2t.shape)
    score_matrix_v2t = torch.full((len(data_loader.dataset.video), len(texts)), -100.0).to(device)

    num_tasks = utils.get_world_size()
    rank = utils.get_rank()
    step = sims_matrix_v2t.size(0) // num_tasks + 1
    start = rank * step
    end = min(sims_matrix_v2t.size(0), start + step)
    # print(sims_matrix_v2t.shape)

    for i, sims in enumerate(metric_logger.log_every(sims_matrix_v2t[start:end], 50, 'V2T ' + header)):
        if args.k_test_batch_size >= config['k_test']:
            topk_sim, topk_idx = sims.topk(k=config['k_test'], dim=0)
            if args.low_resource_eval:
                encoder_output = torch.from_numpy(mmap_file[start + i]).repeat(config['k_test'], 1, 1).to(device)
            else:
            ...
            for j in range(0, config['k_test'], args.k_test_batch_size):
                ...
                if args.low_resource_eval:
                    encoder_output = torch.from_numpy(mmap_file[start + i]).repeat(len(topk_idx), 1, 1).to(device)
                else:
    ...
    step = sims_matrix_t2v.size(0) // num_tasks + 1
    start = rank * step
    end = min(sims_matrix_t2v.size(0), start + step)

    for i, sims in enumerate(metric_logger.log_every(sims_matrix_t2v[start:end], 50, 'T2V ' + header)):
        if args.k_test_batch_size >= config['k_test']:
            topk_sim, topk_idx = sims.topk(k=config['k_test'], dim=0)
            topk_idx = topk_idx.cpu()
            if args.low_resource_eval:
                encoder_output = torch.from_numpy(mmap_file[topk_idx]).to(device, non_blocking=True)
            else:
    ...
    if args.distributed:
        ...
        # torch.distributed.all_reduce(sims_matrix_v2t, op=torch.distributed.ReduceOp.SUM)
        # torch.distributed.all_reduce(sims_matrix_t2v, op=torch.distributed.ReduceOp.SUM)

    total_time = time.time() - start_time
    total_time_str = str(datetime.timedelta(seconds=int(total_time)))
    print('Evaluation time {}'.format(total_time_str))

    return score_matrix_v2t.cpu().numpy(), score_matrix_t2v.cpu().numpy(), sims_matrix_v2t.cpu().numpy(), sims_matrix_t2v.cpu().numpy()
...
def speed_test(model, test_dataloader, device, config):
    ...
    with torch.no_grad():
        warmup_step = 5
        for bid, (video, vid_id) in enumerate(test_dataloader):
            if video.dim() == 4:
                video = video.unsqueeze(1)
            video = video.permute(0, 2, 1, 3, 4)
            B, C, N, H, W = video.size()
            video = rearrange(video, 'b c n h w -> (b n) c h w')
            video = video.to(device, non_blocking=True)
            video_feat, _ = model.visual_encoder(video, config['num_frm_test'])
            if bid >= warmup_step:
                break
        ...
        print("inference speed: {:.2f} ms/batch".format(duration * 1000))
        print("Throughout: {:.2f} videos/s".format(1 / duration * config['batch_size_test']))
...
def main(args, config):
    ...
    model = tome_retrieval(pretrained=config['pretrained'], image_size=config['image_size'], vit=config['vit'],
    ...
    if args.distributed:
        if args.mixed_precision_method != 'deepspeed':
            ...
            model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu], static_graph=static_graph, find_unused_parameters=True)
    ...
    if args.speed_test:
        speed_test(model, test_loader, device, config)
        return 
    ...
    for epoch in range(0, config['max_epoch']):
        ...
        model = neuron_merge(model, config['weime_r'] // config['max_epoch'], config['weime_layers'])
        optimizer = torch.optim.AdamW(params=model.parameters(), lr=config['init_lr'], weight_decay=config['weight_decay'])
        ...
        if not args.evaluate:
            ...
            score_val_v2t, score_val_t2v, sims_matrix_v2t, sims_matrix_t2v = evaluation(model_without_ddp, val_loader, device, config, args)

            if utils.is_main_process():
                print('Validation')
                val_result = vtm_eval(score_val_v2t, score_val_t2v, val_loader.dataset.txt2video, val_loader.dataset.video2txt)
                # val_result = vtm_eval(sims_matrix_v2t, sims_matrix_t2v, val_loader.dataset.txt2video, val_loader.dataset.video2txt)
                print('Val result: ', val_result)

                if val_result['vid_r1'] > best:
    ...
    #         model_without_ddp, _ = load_checkpoint(model_without_ddp, os.path.join(args.output_dir, f'checkpoint_{i}.pth'))
    #     score_test_v2t, score_test_t2v, sims_matrix_v2t, sims_matrix_t2v = evaluation(model_without_ddp, val_loader, device, config, args)
    ...
    #             test_result = vtm_eval(score_test_v2t, score_test_t2v, val_loader.dataset.txt2video, val_loader.dataset.video2txt)
    #             # test_result = vtm_eval(sims_matrix_v2t, sims_matrix_t2v, test_loader.dataset.txt2video, test_loader.dataset.video2txt)
    #             print('Test result: ', test_result)
    #             log_stats = {**{f'val_{k}': v for k, v in test_result.items()},
    #                         'best_epoch': best_epoch,
    #                         }
    ...
    score_test_v2t, score_test_t2v, sims_matrix_v2t, sims_matrix_t2v = evaluation(model_without_ddp, test_loader, device, config, args)
