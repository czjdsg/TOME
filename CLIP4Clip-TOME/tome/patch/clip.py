...
class ToMeResidualAttentionBlock(ResidualAttentionBlock):
    ...
    def frame_flatten_operation(self, x, video_frame, num_cls=1, type="as_patch", permute=True):
        # when input shape = L,B,C, permute to B,L,C first
        if permute:
            x = x.permute(1, 0, 2).contiguous()
        bsz, num_token, hidden_dim = x.shape # BT,L,C
        ...
        if type == "as_patch":
            ...
            x = x.view(bsz // 2, 2, num_token, -1).transpose(1, 2).contiguous().view(bsz // 2, num_token * 2, -1)
    ...
    def frame_flatten_plus(self, x, metric, attn_weights, video_frame):
        ...
        if self._tome_info["hierarchy"] and self._tome_info["layer_idx"] in self._tome_info["frame_flatten_layer"]:
            x = self.frame_flatten_operation(x, video_frame, num_cls=num_cls, type=type)
            metric = self.frame_flatten_operation(metric, video_frame, num_cls=num_cls, type=type, permute=False)
            if attn_weights is not None:
                attn_weights = attn_weights[:, :num_cls].mean(1).squeeze(-1) # B,L
                attn_weights = self.frame_flatten_operation(attn_weights, video_frame, num_cls=num_cls, type=type, permute=False)
            if self._tome_info["size"] is not None:
                self._tome_info["size"] = self.frame_flatten_operation(self._tome_info["size"], video_frame, num_cls=num_cls, type=type)
            self._tome_info["num_cls"] *= 2
            video_frame = video_frame // 2
        return x, metric, attn_weights, video_frame

    def token_merging(self, x, metric, attn_weights, video_frame, return_metric=False):
        # import pdb; pdb.set_trace()
        r = self._tome_info["r"].pop(0)
        num_token = x.shape[0]
        average_type = self._tome_info["average_type"]
        frame_merge = self._tome_info["hierarchy"] and self._tome_info["layer_idx"] in self._tome_info["frame_flatten_layer"]
        merge_type = self._tome_info["type"] if not frame_merge else "soft_f_multi_cls" 
        ...
        if r > 0:
            ...
            if return_metric:
                x, self._tome_info["size"], metric = merge_wavg(merge, x, self._tome_info["size"], metric, type=average_type, size_mode=size_mode)
                return x, metric
            else:
                x, self._tome_info["size"] = merge_wavg(merge, x, self._tome_info["size"], type=average_type, size_mode=size_mode)
        return x

    def token_sharing_and_merging(self, x, metric, attn_weights, video_frame):
        if self._tome_info["layer_idx"] in self._tome_info["frame_flatten_layer"]:
            x, metric = self.token_merging(x, metric, attn_weights, video_frame, return_metric=True)
            x, metric, attn_weights, video_frame = self.frame_flatten_plus(x, metric, attn_weights, video_frame)
    ...
    def forward(self, x_dict:dict):
        ...
        x_attn, metric, attn_weights = self.attention(self.ln_1(x), attn_size)
        x = x + x_attn

        # 太奇怪了，先flatten，再merge，再过MLP，速度要慢12ms，原因可能是多进行了一次两帧内的合并。需要再看一下
        # setting1:
        shape1 = x.shape
        x, video_frame = self.token_sharing_and_merging(x, metric, attn_weights, video_frame)
        shape2 = x.shape

        print("layer {}: {} ---> {}".format(self._tome_info["layer_idx"], shape1, shape2))

        x = x + self.mlp(self.ln_2(x))
        
        # setting2:
        # x, video_frame = self.frame_flatten(x, video_frame, num_cls=self._tome_info["num_cls"], type=self._tome_info["frame_flatten_type"])
