...
# from modules.eva_clip.factory import create_model_and_transforms
# from modules.eva_clip.model import convert_weights_to_lp
...
class CLIP4ClipPreTrainedModel(PreTrainedModel, nn.Module):
    ...
    def from_pretrained(cls, cross_model_name, state_dict=None, cache_dir=None, type_vocab_size=2, *inputs, **kwargs):
        ...
        if task_config.eva_clip:
            show_log(task_config, "Replacing CLIP with EVA-CLIP")
            eva_clip, _, preprocess = create_model_and_transforms("EVA02-CLIP-B-16", "/datahdd/zjcai/CLIP4Clip-tome/modules/eva_clip/EVA02_CLIP_B_psz16_s8B.pt", force_custom_clip=True, precision='fp16')
            # eva_clip, _, preprocess = create_model_and_transforms("EVA02-CLIP-L-14", "/datahdd/zjcai/CLIP4Clip-tome/modules/eva_clip/EVA02_CLIP_L_psz14_s4B.pt", force_custom_clip=True, precision='fp16')
            model.clip = eva_clip
            # convert_weights_to_lp(model.clip)
            # print(model.clip.text)
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        if os.getenv("enable_frame_pos_embed", False):
            self.frame_positional_embeddings = nn.Embedding(cross_config.max_position_embeddings, vision_width).half()
    ...
    def forward(
        ...
        sequence_output, visual_output, gf_sims, visual_hidden_states = self.get_sequence_visual_output(
            ...
            return_hidden_states=hasattr(self, "atp"),
            )
        ...
        if self.training:
            ...
            if gf_sims is not None:
                gf_sim_tgt = self.construct_gf_sim_target(sequence_output, visual_output, video_mask)
                gf_sims = gf_sims + (1 - video_mask.unsqueeze(1)) * -1e4
                loss_gf = nn.functional.kl_div(torch.log_softmax(gf_sims / self.gf_temp, dim=-1), gf_sim_tgt.expand_as(gf_sims), reduction="batchmean")
                # loss_gf = - (gf_sim_tgt * torch.log_softmax(gf_sims / self.gf_temp, dim=-1)).sum(-1).mean()
                loss += loss_gf
            if hasattr(self, "atp"):
                assert visual_hidden_states is not None
                atp_input = visual_hidden_states[self.atp_input_layer][0].view(b, video_frame, -1).transpose(0, 1) # L,BT,C ==> T,B,C
                atp_output = self.atp((atp_input.float(), video_frame))[0].transpose(0, 1) # T,B,C ==> B,T,C
                atp_output = self.atp_linear(atp_output).squeeze(-1) # B,T
                gf_sim_tgt = self.construct_gf_sim_target(sequence_output, visual_output, video_mask).squeeze(1)
                gf_sims = atp_output + (1 - video_mask) * -1e4
                loss_gf = nn.functional.kl_div(torch.log_softmax(gf_sims, dim=-1), gf_sim_tgt, reduction="batchmean")
                # loss_gf = - (gf_sim_tgt * torch.log_softmax(gf_sims / self.gf_temp, dim=-1)).sum(-1).mean()
                loss += loss_gf
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1, frame_selection=False, return_gf_sims=False, return_hidden_states=False):
        ...
        if os.getenv("enable_frame_pos_embed", False):
            ...
            frame_position_embeddings = self.frame_positional_embeddings(position_ids) # B,T,C
        else:
        ...
        if not self.task_config.eva_clip:
            ...
            if hasattr(self.clip.visual, "_tome_info") and self.clip.visual._tome_info["num_cls"] > 1:
                ...
                # visual_hidden = torch.mean(visual_hidden[:, :self.clip.visual._tome_info["num_cls"]].contiguous(), dim=1, keepdim=True)
                # visual_hidden = visual_hidden.view(bs_pair, -1, visual_hidden.shape[-1])
            else:
    ...
    def get_similarity_logits(self, sequence_output, visual_output, attention_mask, video_mask, shaped=False, loose_type=False):
        ...
        # print(visual_output.shape, attention_mask.shape)
        if loose_type:
