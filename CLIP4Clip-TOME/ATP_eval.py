...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument('--tome_r', type=int, default=2, help='Num of token to merge per layer')
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(test_dataloader):
            ...
            if multi_sentence_:
                ...
                # select frames according to gt caption
                Bt, _, _ = sequence_output.shape
                Bv, L, C = visual_output.shape
                sequence_output = sequence_output / sequence_output.norm(dim=-1, keepdim=True)  # Bt,1,C
                visual_output = visual_output / visual_output.norm(dim=-1, keepdim=True)  # Bv,T,C
                t2v_sim = torch.matmul(sequence_output, visual_output.transpose(-1, -2)) # Bt,1,T
                t2v_sim = t2v_sim * video_mask # B,1,T
                sim_topk_idx = t2v_sim.squeeze(1).topk(1, dim=-1)[1] # B,topk
                visual_output = visual_output.gather(dim=1, index=sim_topk_idx.unsqueeze(-1).repeat(1, 1, C))
                video_mask = video_mask.gather(dim=-1, index=sim_topk_idx.unsqueeze(1))
                batch_sequence_output_list.append(sequence_output)
                batch_list_t.append((input_mask, segment_ids,))
