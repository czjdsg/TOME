...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument("--eva_clip", action='store_true', help="Whether to use eva-clip")
...
def prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, local_rank, coef_lr=1.):
    ...
    #     print(n) 
    # print(len(decay_clip_param_tp), len(decay_noclip_param_tp), len(no_decay_clip_param_tp), len(no_decay_noclip_param_tp))
    weight_decay = 0.2
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        # if video_mask.sum() == video_mask.shape[0] * video_mask.shape[-1]:
        #     pass
        # else:
        #     import pdb; pdb.set_trace()
        loss, loss_gf = model(input_ids, segment_ids, input_mask, video, video_mask, args.frame_selection)
        if n_gpu > 1:
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % log_step == 0 and local_rank == 0:
                if loss_gf is None:
                    ...
                    logger.info("Epoch: %d/%s, Step: %d/%d, Lr: %s, Loss: %f, Loss GF: %f, Time/step: %f", epoch + 1,
                                args.epochs, step + 1,
                                len(train_dataloader), "-".join([str('%.9f'%itm) for itm in sorted(list(set(optimizer.get_lr())))]),
                                float(loss),
                                float(loss_gf),
    ...
    if args.do_train:
        ...
        for epoch in range(resumed_epoch, args.epochs):
            ...
            #     logger.info("Model parameters after merging {}".format(sum(p.numel() for p in model.parameters())))
                ...
                # if epoch == 2:
                #     model = neuron_merge(model, args.weime_r, args.weime_layers)
                #     optimizer, scheduler, model = prep_optimizer(args, model, num_train_optimization_steps, device, n_gpu, args.local_rank, coef_lr=coef_lr)
            # model_to_save = model.module if hasattr(model, 'module') else model
            # output_model_file = os.path.join(
            #     args.output_dir, "pytorch_model.bin.{}{}".format("", epoch))
            # optimizer_state_file = os.path.join(
            #     args.output_dir, "pytorch_opt.bin.{}{}".format("", epoch))
            # torch.save(model_to_save.state_dict(), output_model_file)
            # torch.save({
            #         'epoch': epoch,
            #         'optimizer_state_dict': optimizer.state_dict(),
        ...
        if args.enable_weime:
            logger.info("merging model parameters...")
            logger.info("Model parameters before merging {}".format(sum(p.numel() for p in model.parameters() if p.requires_grad)))
            

            model = neuron_merge(args, model, args.weime_r_neuron, device, train_dataloader, args.weime_layers)
            model = attention_head_merge(args, model, args.weime_r_head, device, train_dataloader, args.weime_layers)

            logger.info("Model parameters after merging {}".format(sum(p.numel() for p in model.parameters() if p.requires_grad)))
