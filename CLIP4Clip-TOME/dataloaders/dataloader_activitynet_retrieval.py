...
class ActivityNet_DataLoader(Dataset):
    def __init__(
        ...
        # frames are pre-extracted and saved in the dir named by video_id
        pseudo_video_id_set = set(pseudo_video_id_list)
        for video_file in os.listdir(self.features_path):
            video_id_ = video_file
            if video_id_.split('.')[0] not in pseudo_video_id_set:
                continue
        ...
        for video_id in self.pseudo_video_id_list:
            if video_id not in self.pseudo_caption_dict or video_id not in self.video_dict:
                continue
            caption = self.pseudo_caption_dict[video_id]
            n_caption = len(caption['start'])
            for sub_id in range(n_caption):
                self.iter2video_pairs_dict[len(self.iter2video_pairs_dict)] = (video_id, sub_id)

        self.rawVideoExtractor = RawVideoExtractor[video_reader](framerate=feature_framerate, size=image_resolution, is_train=True if self.subset == "train" else False)
    ...
    def __getitem__(self, feature_idx):
        ...
        video, video_mask = self._get_rawvideo(self.pseudo_video_id_list[idx], starts, ends)
        return pairs_text, pairs_mask, pairs_segment, video, video_mask


class ActivityNet_DataLoaderFrame(ActivityNet_DataLoader):
    def _get_rawvideo(self, idx, s, e):
        video_mask = np.zeros((len(s), self.max_frames), dtype=np.long)
        max_video_length = [0] * len(s)
        ...
        try:
            for i in range(len(s)):
                ...
                # Should be optimized by gathering all asking of this video
                raw_video_data = self.rawVideoExtractor.get_video_data(video_path, start_time, end_time, self.max_frames, self.slice_framepos)
                raw_video_data = raw_video_data['video']
