...
class DiDeMo_DataLoader(Dataset):
    def __init__(
        ...
        with open(video_id_path_dict[self.subset], 'r') as fp:
            video_ids = [itm.strip().split(".")[0] + ".mp4" for itm in fp.readlines()]
            print(video_ids[:10])
        ...
        for root, dub_dir, video_files in os.walk(self.features_path):
            for video_file in video_files:
                video_id_ = video_file.split(".")[0] + ".mp4"
...
class DiDeMo_DataLoaderDecord(DiDeMo_DataLoader):
    def _get_rawvideo(self, idx, s, e):
        video_mask = np.zeros((len(s), self.max_frames), dtype=np.long)
        max_video_length = [0] * len(s)
        ...
        try:
            for i in range(len(s)):
                ...
                cache_id = "{}_{}_{}".format(video_path, start_time, end_time)
                # Should be optimized by gathering all asking of this video
                raw_video_data = self.rawVideoExtractor.get_video_data(video_path, start_time, end_time, max_frames=self.max_frames, slice_type=self.slice_framepos)
                raw_video_data = raw_video_data['video']
