...
from dataloaders.dataloader_msrvtt_retrieval import MSRVTT_DataLoader # MSRVTT_DataLoaderPadding
from dataloaders.dataloader_msrvtt_retrieval import MSRVTT_TrainDataLoader, MSRVTT_TrainDataLoaderDecord #MSRVTT_TrainDataLoaderDecordPadding
from dataloaders.dataloader_msvd_retrieval import MSVD_DataLoader, MSVD_DataLoaderDecord
from dataloaders.dataloader_lsmdc_retrieval import LSMDC_DataLoader, LSMDC_DataLoaderDecord
from dataloaders.dataloader_activitynet_retrieval import ActivityNet_DataLoader, ActivityNet_DataLoaderFrame
from dataloaders.dataloader_didemo_retrieval import DiDeMo_DataLoader, DiDeMo_DataLoaderDecord

def dataloader_msrvtt_train(args, tokenizer):
    msrvtt_dataset = MSRVTT_TrainDataLoaderDecord(
        ...
        padding_base=args.padding_base,
    )
...
def dataloader_msvd_train(args, tokenizer):
    dataloader_class = MSVD_DataLoader if args.video_reader == "cv2" else MSVD_DataLoaderDecord
    msvd_dataset = dataloader_class(
        subset="train",
...
def dataloader_lsmdc_train(args, tokenizer):
    dataloader_class = LSMDC_DataLoader if args.video_reader == "cv2" else LSMDC_DataLoaderDecord
    lsmdc_dataset = dataloader_class(
        subset="train",
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
...
def dataloader_lsmdc_test(args, tokenizer, subset="test"):
    dataloader_class = LSMDC_DataLoader if args.video_reader == "cv2" else LSMDC_DataLoaderDecord
    lsmdc_testset = dataloader_class(
        subset=subset,
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
...
def dataloader_activity_train(args, tokenizer):
    activity_dataset = ActivityNet_DataLoaderFrame(
        subset="train",
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
...
def dataloader_activity_test(args, tokenizer, subset="test"):
    activity_testset = ActivityNet_DataLoaderFrame(
        subset=subset,
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
...
def dataloader_didemo_train(args, tokenizer):
    didemo_dataset = DiDeMo_DataLoaderDecord(
        subset="train",
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
...
def dataloader_didemo_test(args, tokenizer, subset="test"):
    didemo_testset = DiDeMo_DataLoaderDecord(
        subset=subset,
        data_path=args.data_path,
        features_path=args.features_path,
        max_words=args.max_words,
